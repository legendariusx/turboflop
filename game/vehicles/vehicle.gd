...
func _physics_process(delta: float) -> void:
	...
	elif not _is_accellerating:
		brake = BRAKE_SOFT_STRENGTH
	else:
		brake = 0.0
	
	# set engine force
	if _is_accellerating and not _is_braking:
		var force = force_axis * accelleration_force
