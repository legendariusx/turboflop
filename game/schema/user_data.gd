...
static func set_user_data(u_position: Vector3, u_rotation: Vector3, u_linear_velocity: Vector3, u_angular_velocity: Vector3, u_is_active: bool, u_track_id: int):
	if not SpacetimeDB.is_connected_db():
		await SpacetimeDB.connected
	SpacetimeDB.call_reducer("set_user_data", [u_position, u_rotation, u_linear_velocity, u_angular_velocity, u_is_active, u_track_id])
